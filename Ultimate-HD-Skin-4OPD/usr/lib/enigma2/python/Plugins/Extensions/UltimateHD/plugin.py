...
config.plugins.Ultimate = ConfigSubsection()
def Plugins(**kwargs):
...
class Ultimate_Config(Screen, ConfigListScreen):

    skin = """
            <screen name="Ultimate_Config" position="center,center" size="1280,720" title="Ultimate Setup" >
    ...
    def keyYellow(self):
        if self.myAtileHD_active.value:
            self.session.openWithCallback(self.UltimateScreenCB, UltimateScreens)
        else:
    ...
    def about(self):
        self.session.open(Ultimate_About)
...
class UltimateScreens(Screen):
    ...
    skin = """
            <screen name="UltimateScreens" position="center,center" size="1280,720" title="Ultimate Setup">
                    <widget source="Title" render="Label" position="70,47" size="950,43" font="Regular;35" transparent="1" />
